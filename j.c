...
static void process_midi_input(jack_port_t *port, jack_nframes_t nframes) {
	...
	for (int i = 0; i < events; i++) {
		...
		jack_time_t ftime = jack_frames_to_time(g_client, event.time);
		...
		_jtime = jtime;
		_ftime = ftime;
		printf("#%d bytes=%d jtime=%" PRIu64 "(%" PRIu64 ") ftime=%" PRIu64 "(%" PRIu64 "): ", count, size, (uint64_t)jtime, (uint64_t)djtime, (uint64_t)ftime, (uint64_t)dftime);
#else
...
int main(int argc, char *argv[]) {
	...
	// to start, just put the note freq of the learn key
